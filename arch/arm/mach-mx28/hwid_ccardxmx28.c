...
void ccardxmx28_set_hwid(u32 low, u32 high)
{
	u32 cust0, cust1;
...
int ccardxmx28_create_sysfs_entries(void)
{
	...
	ret = sysfs_create_file(ccardxmx28_kobj, &hwid_tf_attr.attr);
	if (ret) {
		printk(KERN_ERR
	...
	ret = sysfs_create_file(ccardxmx28_kobj, &hwid_variant_attr.attr);
	if (ret) {
		printk(KERN_ERR
	...
	ret = sysfs_create_file(ccardxmx28_kobj, &hwid_hv_attr.attr);
	if (ret) {
		printk(KERN_ERR
	...
	ret = sysfs_create_file(ccardxmx28_kobj, &hwid_cert_attr.attr);
	if (ret) {
		printk(KERN_ERR
	...
	ret = sysfs_create_file(ccardxmx28_kobj, &hwid_year_attr.attr);
	if (ret) {
		printk(KERN_ERR
	...
	ret = sysfs_create_file(ccardxmx28_kobj, &hwid_month_attr.attr);
	if (ret) {
		printk(KERN_ERR
	...
	ret = sysfs_create_file(ccardxmx28_kobj, &hwid_sn_attr.attr);
	if (ret) {
		printk(KERN_ERR
