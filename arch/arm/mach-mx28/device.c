...
#include "mx28_cpx2.h"
#elif defined(CONFIG_MODULE_CCARDXMX28)
...
#elif defined(CONFIG_MACH_CCARDWMX28JS)
...
#else /*#elif defined(CONFIG_MACH_CCARDWMX28JS)*/
...
static void __init mx28_init_spi(void)
{
	...
	mxs_add_device(pdev, 3);
#endif /* CONFIG_CCARDXMX28_SPI_SSP1_ENABLE */
	...
	mxs_add_device(pdev, 3);
#endif /* CONFIG_CCARDXMX28_SPI_SSP3_ENABLE */
}
...
static struct fec_platform_data fec_pdata0 = {
	.phy = PHY_INTERFACE_MODE_RMII,
	.init = mx28_ccardxmx28_enet_gpio_init,
#if defined(CONFIG_CCARDXMX28_ENET0_LEDS)
...
#if !defined(CONFIG_MODULE_CCARDXMX28) || \
    (defined(CONFIG_MODULE_CCARDXMX28) && defined(CONFIG_CCMX28_CAN0))
	mxs_add_device(pdev, 2);
#endif
...
#if !defined(CONFIG_MODULE_CCARDXMX28) || \
    (defined(CONFIG_MODULE_CCARDXMX28) && defined(CONFIG_CCMX28_CAN1))
	mxs_add_device(pdev, 2);
#endif
}
#else
...
mx28_persistent_bit_config[] = {
	...
	{ .reg = 2, .start =  0, .width = 2,
		.name = "boot_attempts" },
	{ .reg = 2, .start =  2, .width = 30,
		.name = "SPARE_2" },
#elif defined(CONFIG_MODULE_CCARDXMX28)
	{ .reg = 2, .start =  0, .width = 2,
		.name = "boot_attempts" },
	{ .reg = 2, .start =  2, .width = 22,
		.name = "SPARE_2" },
	{ .reg = 2, .start =  24, .width = 8,
		.name = "DIGI_BOOT_LOADER_RESERVED" },
#else
