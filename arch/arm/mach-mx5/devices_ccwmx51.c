...
{
	u8 variant = ccxmx5x_get_mod_variant();

	if (variant == 0x02 || variant == 0x06 || variant == 0x08 ||
			variant == 0x0c || variant == 0x0e || variant == 0x14 ||
			variant == 0x16 || variant == 0x03 || variant == 0x07 ||
			variant == 0x09 || variant == 0x0d || variant == 0x11 ||
			variant == 0x13 )
		return CCXMX51_800;
	else if( variant == 0x04 || variant == 0x0a || variant == 0x10 ||
			variant == 0x12 || variant == 0x05 || variant == 0x0b ||
			variant == 0x0f || variant == 0x15 )
		return CCXMX51_600;
	else{
		printk(KERN_ERR"Incorrect variant programmed (%x)\n",variant);
	}
	return -1;
}
...
static struct gpio_keys_button ccwmx51js_gpio_keys[] = {
		GPIO_BUTTON_LOW(DIO0_POWER_KEY,	KEY_POWER,	"power"),
		GPIO_BUTTON_LOW(DIO1_SEARCH_KEY,	KEY_SEARCH,	"search"),
		GPIO_BUTTON_LOW(DIO3_POWER_DELAYED_KEY,	KEY_POWER2,	"powerd"),
		GPIO_BUTTON_LOW(DIO4_HOME_KEY,	KEY_HOME,	"home"),
		GPIO_BUTTON_LOW(DIO5_MENU_KEY,	KEY_MENU,	"menu"),
		GPIO_BUTTON_LOW(DIO6_BACK_KEY,	KEY_BACK,	"back"),
};
#else
static struct gpio_keys_button ccwmx51js_gpio_keys[] = {
		GPIO_BUTTON_LOW(DIO0_POWER_KEY,	KEY_POWER,	"power"),
...
#endif /* CONFIG_CCXMX5X_DISP0 && CONFIG_CCXMX5X_DISP1 */

void ccwmx51_set_lcd_struct_from_bl_shared(nv_lcd_config_t *lcd_config)
{
	int i;
	...
	for (i = 0; i < ARRAY_SIZE(lcd_panel_list); i++) {
		if (!strncmp(lcd_panel_list[i].fb_pdata.mode_str,
			...
			struct fb_videomode *videmode = lcd_panel_list[i].fb_pdata.mode;

			if ( lcd1_valid ) {
				/* Skip the name field */
				memcpy(&videmode->refresh, &lcd_config->lcd1.video_mode.refresh,
			...
			if ( lcd2_valid ) {
				/* Skip the name field */
				memcpy(&videmode->refresh, &lcd_config->lcd2.video_mode.refresh,
