...
void ccwmx51_gpio_dio_init(void)
{
#if defined(CONFIG_JSCCWMX51_V1)
#if !defined(CONFIG_PATA_FSL) && !defined(CONFIG_PATA_FSL_MODULE)
...
#endif
#endif //defined(CONFIG_JSCCWMX51_V1)
...
#if defined(CONFIG_KEYBOARD_CCWMX5X_DIO_KEYS)
#if !defined (CONFIG_SPI_MXC_SELECT2)
	mxc_iomux_set_pad(MX51_PIN_NANDF_RB3,
			PAD_CTL_DRV_HIGH | PAD_CTL_ODE_OPENDRAIN_NONE | PAD_CTL_100K_PU | PAD_CTL_PUE_PULL | PAD_CTL_PKE_ENABLE );
	/* Configure power key as wake up */
	set_irq_wake(gpio_to_irq(IOMUX_TO_GPIO(MX51_PIN_NANDF_RB3)),1);
#endif

#if !defined(CONFIG_PATA_FSL) && !defined(CONFIG_PATA_FSL_MODULE)
	mxc_iomux_set_pad(MX51_PIN_NANDF_CS2,
			PAD_CTL_DRV_HIGH | PAD_CTL_ODE_OPENDRAIN_NONE | PAD_CTL_100K_PU | PAD_CTL_PUE_PULL | PAD_CTL_PKE_ENABLE);
	mxc_iomux_set_pad(MX51_PIN_NANDF_CS4,
			PAD_CTL_DRV_HIGH | PAD_CTL_ODE_OPENDRAIN_NONE | PAD_CTL_100K_PU | PAD_CTL_PUE_PULL | PAD_CTL_PKE_ENABLE);
	/* Configure home key as wake up */
	set_irq_wake(gpio_to_irq(IOMUX_TO_GPIO(MX51_PIN_NANDF_CS4)),1);
	mxc_iomux_set_pad(MX51_PIN_NANDF_CS5,
			PAD_CTL_DRV_HIGH | PAD_CTL_ODE_OPENDRAIN_NONE | PAD_CTL_100K_PU | PAD_CTL_PUE_PULL | PAD_CTL_PKE_ENABLE);
	/* Configure menu key as wake up */
	set_irq_wake(gpio_to_irq(IOMUX_TO_GPIO(MX51_PIN_NANDF_CS5)),1);
	mxc_iomux_set_pad(MX51_PIN_NANDF_CS6,
			PAD_CTL_DRV_HIGH | PAD_CTL_ODE_OPENDRAIN_NONE | PAD_CTL_100K_PU | PAD_CTL_PUE_PULL | PAD_CTL_PKE_ENABLE);
#endif

#if !defined(CONFIG_SPI_MXC_SELECT2) || (!defined(CONFIG_PATA_FSL) && !defined(CONFIG_PATA_FSL_MODULE))
	mxc_iomux_set_pad(MX51_PIN_NANDF_RB1,
			PAD_CTL_DRV_HIGH | PAD_CTL_ODE_OPENDRAIN_NONE | PAD_CTL_100K_PU | PAD_CTL_PUE_PULL | PAD_CTL_PKE_ENABLE);
...
#if !defined(CONFIG_SPI_MXC_SELECT2) || (!defined(CONFIG_PATA_FSL) && !defined(CONFIG_PATA_FSL_MODULE))
	gpio_request(IOMUX_TO_GPIO(MX51_PIN_NANDF_RB1), "nandf_rb1");
	gpio_direction_output(IOMUX_TO_GPIO(MX51_PIN_NANDF_RB1), 1);
	gpio_set_value(IOMUX_TO_GPIO(MX51_PIN_NANDF_RB1), 1);
#endif
#else
	/* Configure menu key (user key 2) as wake up */
	set_irq_wake(gpio_to_irq(IOMUX_TO_GPIO(MX51_PIN_GPIO1_1)),1);
	/* Configure home key (user key 1) as wake up */
	set_irq_wake(gpio_to_irq(IOMUX_TO_GPIO(MX51_PIN_DISPB2_SER_DIO)),1);
#endif	//defined(CONFIG_KEYBOARD_CCWMX5X_DIO_KEYS)

#endif //defined(CONFIG_ANDROID)

#endif // defined(CONFIG_JSCCWMX51_V2)
}
