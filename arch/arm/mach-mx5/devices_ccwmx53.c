...
static struct gpio_keys_button ccwmx53js_gpio_keys[] = {
...
struct gpio_keys_platform_data ccwmx53js_gpio_key_info = {
	.buttons	= ccwmx53js_gpio_keys,
	.nbuttons	= ARRAY_SIZE(ccwmx53js_gpio_keys),
};

struct platform_device ccwmx53js_keys_gpio = {
	.name	= "gpio-keys",
	.id	= -1,
};
#endif // KEYBOARD_GPIO
...
void ccwmx53_set_lcd_struct_from_bl_shared(nv_lcd_config_t *lcd_config)
{
	int i;
...
int ccxmx53_check_display_type(int dispif, const char *str)
{
#if defined(CONFIG_JSCCWMX53_V2)
	char *options = NULL;
	int len = strlen(str);
	int ret = 0;
#if defined(CONFIG_CCXMX5X_DISP0)
...
int ccwmx53_smsc911x_init( void )
{
	int ret;
...
__init void ccwmx53_register_fusion_touch(void)
{
