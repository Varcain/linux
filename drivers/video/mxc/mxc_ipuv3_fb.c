...
static int mxcfb_check_var(struct fb_var_screeninfo *var, struct fb_info *info)
{
	...
	/*
	 * As shown in the IMX51 datasheet, Advanced DMA (ADMA) supports transfers
	 * of 4-KB-aligned data. Apparently the GPU inherits this limitation so that
	 * the graphical virtual memory must also be aligned.
	 *
	 * That's why we force a 32-bit alignment in the x virtual resolution and a
	 * 128-bit alignment in the y virtual resolution (32 * 128 = 4096 = 4K).
	 *
	 * These values, 32 and 128, are the ones used in some user-space libraries
	 * (i.e. libgralloc and libhardware) to align virtual buffers precisely
	 * because of the same reason. Anyway, this limitation is not considered
	 * in other user space accesses to these buffers. That's why the alignment
	 * has to be assured here too.
	 */
#define ALIGN_PIXEL_32(x)		((x + 31) & ~31)
#define ALIGN_PIXEL_128(x)		((x + 127) & ~127)
	var->xres_virtual = ALIGN_PIXEL_32(var->xres);
