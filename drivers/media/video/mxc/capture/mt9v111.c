...
static int ioctl_s_ctrl(struct v4l2_int_device *s, struct v4l2_control *vc)
{
	...
	case V4L2_CID_MXC_ROT:
		pr_debug("   V4L2_CID_MXC_ROT\n");
		pr_info("This sensor doesn't support rotation\n");
		break;
	case V4L2_CID_MXC_VF_ROT:
		pr_debug("   V4L2_CID_MXC_VF_ROT\n");
		pr_info("This sensor doesn't support rotation\n");
		break;
	default:
		pr_debug("   Default case\n");
...
{
	if (fsize->index > MT9V111_OutputResolution_VGA)  // Digi: Support for VGA only
	  return -EINVAL;

	fsize->pixel_format = mt9v111_data[0].pix.pixelformat;
	fsize->discrete.width = mt9v111_data[0].pix.width;
	fsize->discrete.height = mt9v111_data[0].pix.height;
	return 0;
}

/*!
 * ioctl_g_chip_ident - V4L2 sensor interface handler for VIDIOC_DBG_G_CHIP_IDENT ioctl
 * @s: pointer to standard V4L2 device structure
 * @id: pointer to int
 *
 * Return 0.
...
static int ioctl_g_chip_ident(struct v4l2_int_device *s, int *id)
{
	...
	strcpy(((struct v4l2_dbg_chip_ident *)id)->match.name, "mt9v111_camera");
...
/*	{vidioc_int_queryctrl_num, (v4l2_int_ioctl_func *) ioctl_queryctrl}, */
	...
	// Digi: Funcions added to support Android camera.
